...
public class CommonRec_PSO {
    ...
    public void printResults_v1(int metrics) throws IOException {
    	FileWriter R_fw;
    	FileWriter RHat_fw;
    	if(metrics == CommonRec_PSO.RMSE){
    		R_fw = new FileWriter(new File("./" + dataSetName + "_RMSE_R.txt"), true);
        	RHat_fw = new FileWriter(new File("./" + dataSetName  + "_RMSE_RHat.txt"), true);
    	}else{
    		R_fw = new FileWriter(new File("./" + dataSetName + "_MAE_R.txt"), true);
        	RHat_fw = new FileWriter(new File("./" + dataSetName  + "_MAE_RHat.txt"), true);
    	}
    	
    	double sum = 0;
    	Matrix R = new Matrix(userMaxID, itemMaxID);
    	for (RTuple trainR : trainDataSet){
    		R.set(trainR.userID - 1, trainR.itemID - 1, trainR.rating);
    		sum += trainR.rating;
    	}
    	for (RTuple valR: valDataSet){
    		R.set(valR.userID - 1, valR.itemID - 1, valR.rating);
    		sum += valR.rating;
    	}
    	for (RTuple testR : testDataSet){
    		R.set(testR.userID - 1, testR.itemID - 1, testR.rating);
    		sum += testR.rating;
    	}
    	double size = trainDataSet.size() + valDataSet.size() + testDataSet.size();
    	double avg = sum / size;
    	
    	Matrix P_Mat = new Matrix(P);
    	Matrix Q_Mat = new Matrix(Q);
    	Matrix RHat = P_Mat.times(Q_Mat.transpose());
    	...
    	for(int i=1;i<RHat.getRowDimension();i++){
            for(int j=1;j<RHat.getColumnDimension();j++){
                double val = RHat.get(i,j);
                RHat_fw.write(val + "\t");
                RHat_fw.flush();
            }
            RHat_fw.write("\n");
        }
    	System.out.println(RHat.getRowDimension() + "*" + RHat.getColumnDimension());
    	R_fw.close();
		RHat_fw.close();
    }
    ...
    public void printResults_v2(int metrics, int featureDimension) throws IOException {
    	...
    	for (int userID = 1; userID <= userMaxID; userID++){
    		for (int dim = 0; dim < featureDimension; dim++) {
    			P_fw.write(userID + "::" + dim + "::" + P[userID][dim] + "\n");
    			P_fw.flush();
    		}
    	}
    	...
    	for (int itemID = 1; itemID <= itemMaxID; itemID++){
    		for (int dim = 0; dim < featureDimension; dim++) {
    			Q_fw.write(itemID + "::" + dim + "::" + Q[itemID][dim] + "\n");
    			Q_fw.flush();
    		}
    	}
